function processRequest {
	param ($_comps, 
		   ...
		   [string]$_path="PatchCheckerOut.csv",
    ...
    if ($_listAll) {
        ...
        if($_noConsoleOut) {
            $_comps | ForEach-Object {
                $wmiHash = Get-WmiObject -ComputerName $_ -Class 'Win32_QuickFixEngineering'
            ...
            $_comps | ForEach-Object {
                $wmiHash = Get-WmiObject -ComputerName $_ -Class 'Win32_QuickFixEngineering' # -OutVariable Export
        ...
        if ($groupByHostName.ToUpper() -eq "N" -or [string]::IsNullOrEmpty($groupByHostName)) {
            ...
            $_patches | ForEach-Object {
                ...
                if ($_saveToFile -and $patchCount -eq 1) { $Export | export-CSV -Path $_path -NoTypeInformation }
                elseif ($_saveToFile) { $Export | export-CSV -Path $_path -NoTypeInformation -Append }
	            ...
	            $_comps | select-object @{ n = 'Hostname'; e = {$_}},
		                                ...
		                                @{ n = 'Patches Found' ; e = { 
		                                 ...
		                                 @{ n = 'Date Installed' ; e = { 
                                                                       ...
                                                                       } } -OutVariable Export | format-table *>$null
            }
    ...
    if ($_saveToFile) { $Export | export-CSV -Path $_path -NoTypeInformation }
...
function outputPrompt {
    ...
    $outputMode = read-host -prompt "`nSave To File (1), Console Output (2), or Both (3)"
    ...
    if ($outputMode -eq 1 -or $outputMode -eq 3) {
        ...
        write-output   "*    Leave this entry blank to use the default file name of 'PatchCheckerOut.csv',  *"
        ...
        do { 
            $fileName = read-host -prompt "Save As [Default=PatchCheckerOut.csv]"
        ...
        if (!$fileName -and $outputMode -eq 1) { processRequest $comps $patches $true -_noConsoleOut $true -_listAll $_listAll}
        elseif(!$fileName) { processRequest $comps $patches $true -_listAll $_listAll}
        elseif ($fileName.ToUpper() -ne "B" -and $overWriteConfirmed.ToUpper() -ne "B" -and $outputMode -eq 1) { 
            processRequest $comps $patches $true $fileName $true $_listAll
        }
        elseif ($fileName.ToUpper() -ne "B" -and $overWriteConfirmed.ToUpper() -ne "B") {
            processRequest $comps $patches $true $fileName -_listAll $_listAll
        }
    }

    elseif ($outputMode.ToUpper() -eq 2) { processRequest $comps $patches -_listAll $_listAll }	
}    
...
do {
    ...
    while ($userInput.ToUpper() -ne "Q" -and 
           $userInput -ne 1 -and $userInput -ne 2 -and $userInput -ne 3)

    if ($userInput.ToUpper() -eq "Q" -or $userInput -eq 3) { exit }
    ...
    if ($readFileOrManualEntry -eq 1) {
        ...
        do {
            ...
            if (![string]::IsNullOrEmpty($compsFilePath) -and $compsFilePath.ToUpper() -ne "B" -and $compsFilePath.ToUpper() -ne "Q") { 
                $fileNotFound = $(!$(test-path $compsFilePath))
                if ($fileNotFound) { write-output "`n`tFile Not Found!`n" }
                }
        }
        ...
        if ($compsFilePath.ToUpper() -ne "B" -and $userInput -eq 1) {
            ...
            do {
                ...
                if (![string]::IsNullOrEmpty($patchesFilePath) -and $patchesFilePath.ToUpper() -ne "B" -and $patchesFilePath.ToUpper() -ne "Q") { 
                    $fileNotFound = $(!$(test-path $patchesFilePath))
                    if ($fileNotFound) { write-output "`n`tFile Not Found!`n" }
                }
            }
        ...
        if ($compsFilePath.ToUpper() -ne "B" -and ($userInput -eq 2 -or $patchesFilePath.Toupper() -ne "B")) { $comps = Get-Content $compsFilePath -ErrorAction Stop }
    ...
    if ($readFileOrManualEntry.ToUpper() -ne "B" -and
        ...
        ($userInput -eq 2 -or
            ...
            outputPrompt $comps $patches $listAllPatches
	}
}
