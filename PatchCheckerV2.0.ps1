function processRequest {
	param ($_comps, 
		   ...
		   [string]$_path="PatchCheckerOut-$(Get-Date -Format  MMddyyyy_HHmmss).csv",
           [bool]$_noConsoleOut=$false,
           [bool]$_listAll,
		   [bool]$_complianceCheck)

    if($_saveToFile) { New-Item -Path $_path -ItemType "file" *>$null }
    ...
    if (!$_complianceCheck -and !($groupByHostName.ToUpper() -eq "N" -or [string]::IsNullOrEmpty($groupByHostName)) -and $_saveToFile) {
...
function outputPrompt {
    ...
    if ($outputMode -eq 1 -or $outputMode -eq 3) {
        ...
        write-output   "*                       'PatchCheckerOut-$(Get-Date -Format MMddyyyy_HHmmss).csv',                      *"
        ...
        write-output   "*                                                                                   *"
        write-output   "*     (Time stamp may vary slightly from that shown above and at prompt below,      *"
        write-output   "*                      if the default file name is used.)                           *`n"

        do { 
            $fileName = read-host -prompt "Save As [Default=PatchCheckerOut-$(Get-Date -Format  MMddyyyy_HHmmss).csv]"
        ...
        if (!$fileName -and $outputMode -eq 1) { processRequest $comps $patches $true -_noConsoleOut $true -_listAll $_listAll -_complianceCheck $_complianceCheck}
        elseif(!$fileName) { processRequest $comps $patches $true -_listAll $_listAll -_complianceCheck $_complianceCheck}
