        ...
        if($_noConsoleOut) {
			if ($_complianceCheck -and $forCorporateDashboard) {
				    $_comps | ForEach-Object {
                        Try {
                            $currComp = $_
                            $os = (Get-WmiObject -ComputerName $_ -Class 'Win32_OperatingSystem' -ErrorAction continue).Caption
                            ...
                            Catch { Write-Output "Failed to Query ($currComp): $_.Exception.Message" }
                        } | Sort-Object 'Install Date' -OutVariable Export *>$null
			}
