    ...
    if ($_listAll) {
        ...
        if($_noConsoleOut) {
			if ($_complianceCheck) {
                        ...
                        Try {
					        ...
					        $wmiHash | Select-Object -Property PSComputerName, Description, HotFixID, InstalledBy, InstalledOn |
							           Sort-Object InstalledOn | Select-Object -Last 1 -Property PSComputerName,
                                                                    ...
                                                                    @{ n = "Days Past Due" ; e = { $(if (!($checkDate.adddays(-180) -lt $_.InstalledOn)) { 
                                                                                                        ( New-TimeSpan -Start $_.InstalledOn -End $checkDate.adddays(-180)).Days 
                                                                                                    }
                                                                                                    else { "N/A" }
                                                                                                    )
                                                                                                }
                    ...
                    } | Sort-Object 'Installed On'-OutVariable Export *>$null
			}
			else {	
				$_comps | ForEach-Object {
                    $currComp = $_
